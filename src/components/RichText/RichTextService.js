...
function getProps(type, overrides, data = {}) {
  ...
  return {
    ...overrideProps, ...data, className:
      cx(data.className, overrideProps.className) ||
      undefined
  };
}
...
function DefaultBlock(props) {
  console.log(props);
  return <div style={{ color: 'white', backgroundColor: 'red' }}>No custom component defined.</div>;
};

function DefaultInline(props) {
  console.log(props);
  return <span style={{ color: 'white', backgroundColor: 'red' }}>No custom component defined.</span>;
};
...
function nodeToJsx(node = {}, options, key) {
  ...
  if (isTextElement) {
    ...
    return marks.reduce((children, mark, markKey) => {
      ...
      const props = getProps(mark.type, inlines, { ...data, key: `${key}${markKey}` });
